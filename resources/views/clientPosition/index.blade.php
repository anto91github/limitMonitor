                        ...
                        <tbody>
                            @foreach ($clientLimits as $limit)
                                <tr>
                                    ...
                                    <td
                                        class="{{ $limit['status'] == 'LIMIT REACHED' || $limit['status'] == 'INACTIVE' ? 'text-danger' : '' }}">
                                        {{ $limit['status'] }}
                                    </td>
                                    <td>{{ $limit['update_by'] ?? '-' }}</td>
                                    <td>{{ isset($limit['update_at']) ? \Carbon\Carbon::parse($limit['update_at'])->format('d/m/Y H:i') : '-' }}
                                    </td>
                                    @if (Auth::user()->role_id == 3 || Auth::user()->role_id == 1)
                                        <td>
                                            <a href="{{ route('formclientlimit.edit', $limit['nama_client']) }}"
                                                class="btn btn-sm btn-warning">Edit</a>
                                            <form action="{{ route('clientposition.delete', $limit['nama_client']) }}"
                                                method="POST" style="display:inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit" class="btn btn-sm btn-danger"
                                                    onclick="return confirm('Yakin ingin reset limit ke 0?')">Delete</button>
                                            </form>
                                        </td>
                                    @endif
                                </tr>
